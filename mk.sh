...
YK_PROJECT_NAME_LIST_q811=(
   "Q811_EMMC_YKQ_FWVGA"
   "Q811_37_EMMC_DINGDA_ML5088_HD720"
)

YK_PROJECT_NAME_LIST_q812=(
   "Q812_EMMC_CAIFU9_KS680M_HD720"
)

YK_PROJECT_NAME_LIST_yk668=(
   "YK668_EMMC_YKQ_FWVGA"
   "YK668_EMMC_CHUANXUNDA_S06_HD640"
   "YK668_37_EMMC_CAIFU_KS810A_HD720"
   "YK668_37_EMMC_CAIFU_KS810C_HD720"
   "YK668_37_EMMC_LEPAI_S16_HD720"
   "YK668_37_EMMC_ZHUOXINTE_ZA66_HD720"
)


YK_PROJECT_NAME_LIST_yk736=(
   "YK736_EMMC_YKQ_FWVGA"
   "YK736_EMMC_YITONG_QILIPAI_D1_WVGA"
   "YK736_EMMC_YKQ_HD640_CT6M"
   "YK736_EMMC_YKQ_FWVGA_PLUS_CT6M"
   "YK736_EMMC_CAIFU9_KS937_HDPLUS"
   "YK736_EMMC_CAIFU9_KS937B_HDPLUS"
   "YK736_EMMC_CAIFU9_KS927_HD640"
   "YK736_EMMC_CAIFU9_KS927_HDPLUS"
   "YK736_EMMC_CAIFU9_KS907B_HDPLUS"
   "YK736_EMMC_CAIFU9_KS907E_HDPLUS"
   "YK736_EMMC_CAIFU9_KS907_HD640"
   "YK736_EMMC_CAIFU9_KS907_HDPLUS"
   "YK736_EMMC_CAIFU9_KS939A_HDPLUS"
   "YK736_EMMC_KUANGRE_A17_FWVGA_PLUS_DIS"
...
YK_PROJECT_NAME_LIST_yk739=(
   ...
   "YK739_EMMC_ZHUOXINTE_Z5201M_HD720"
)
...
YK_PROJECT_NAME_LIST_yk831=(
   "YK831_EMMC_YKQ_FWVGA"
   "YK831_EMMC_HEIBAIHUI_GB03_HDPLUS"
)

YK_PROJECT_NAME_LIST_yk670=(
   "YK670_EMMC_JINHUAZHITONG_M5340_FWVGA_PLUS"
)
...
if [ -e out/target/product/$prj_folder_name/obj/BOOTLOADER_OBJ/build-$prj_folder_name/dev/lcm ];then
rm -rf out/target/product/$prj_folder_name/obj/BOOTLOADER_OBJ/build-$prj_folder_name/dev/lcm
fi
if [ -e out/target/product/$prj_folder_name/obj/BOOTLOADER_OBJ/build-$prj_folder_name/dev/lcm0 ];then
rm -rf out/target/product/$prj_folder_name/obj/BOOTLOADER_OBJ/build-$prj_folder_name/dev/lcm0
fi
...
rm -rf out/target/product/$prj_folder_name/obj/KERNEL_OBJ/drivers/misc/mediatek/sensors-1.0/alsps
...
case $prj_name in
        q811_v10_lwg_bsp1g)
            _inlist=(${YK_PROJECT_NAME_LIST_q811[@]})
	;;
        q812_lwg_bsp1g)
            _inlist=(${YK_PROJECT_NAME_LIST_q812[@]})
	;;
        yk668_lwg_bsp1g)
            _inlist=(${YK_PROJECT_NAME_LIST_yk668[@]})
	;;
        yk668_lwg_bsp)
            _inlist=(${YK_PROJECT_NAME_LIST_yk668[@]})
	;;
	...
	;;
	yk831_lwtg_bsp)
	    _inlist=(${YK_PROJECT_NAME_LIST_yk831[@]})
	;;
        yk670_lwg_bsp1g)
	    _inlist=(${YK_PROJECT_NAME_LIST_yk670[@]})
	;;
esac
#zwenbing add start
...
#add by zwl start 20180515
	BUILD_GMS=`grep -w "BUILD_GMS_VERSION" "$cust_file" |tail -1 |cut -d "=" -f 2 |tr -d " "`
		echo -e "\e[1;36mGoogle GMS $BUILD_GMS\e[0m"	
if [ -n "$BUILD_GMS" ];then 
	if [ ! -d "vendor/google/$BUILD_GMS/apps" ];then
		echo 
		echo -e "\e[1;36mGoogle $BUILD_GMS包不存在,请确认。GMS包默认路径为当前工程同级目录:../../Google目录下面\e[0m"
		echo -e "\e[1;36m如果未下载,请下载$BUILD_GMS包，当前命令行输入：\e[0m" 
		echo -e "\e[1;36m    git clone srv@192.168.12.197:/home/srv/data/Google/GMS8.1 ../../Google/GMS8.1\e[0m"
		echo -e "\e[1;36m不需要编译GMS包,请在cust_project.mk中注释BUILD_GMS的定义\e[0m"
		exit 0
	else 
		echo -e "\e[1;36mGoogle GMS is OK\e[0m"	
	fi                           
fi
#add by zwl end 20180515
...
#copy custom files
	...
	MTK_PATCH_FOR_TELCEL=`grep "^MTK_PATCH_FOR_TELCEL" "$cust_file" |tail -1 |cut -d "=" -f 2 |tr -d " "`
	if [ "$MTK_PATCH_FOR_TELCEL" = "yes"];then 
	echo "copy custom files:mtk_patch_for_telcel..."
	cp -R ../mtk_patch_for_telcel/* .
    fi
    ...
    #20180711 zwb add
    ################################################################################################################
    YKQ_DOUJIA_4M_TEE_PATCH=`grep "^YKQ_DOUJIA_4M_TEE_PATCH" "$cust_file" |tail -1 |cut -d "=" -f 2 |tr -d " "`
    if [ "$YKQ_DOUJIA_4M_TEE_PATCH" = "yes" ];then 
        rm device/mediatek/sepolicy/bsp/non_plat/attestation.te
        rm device/mediatek/sepolicy/bsp/non_plat/init_thh.te
        rm device/mediatek/sepolicy/bsp/non_plat/teei_daemon.te
        rm -rf kernel-4.4/drivers/misc/mediatek/teei/Kconfig
        rm -rf vendor/mediatek/proprietary/trustzone/atf/v1.3/services/spd/teeid
        rm -rf vendor/mediatek/proprietary/trustzone/microtrust/source/platform/mt6739
        rm  vendor/mediatek/proprietary/trustzone/microtrust/source/common/270/imsg_log/Android.mk
        rm  vendor/mediatek/proprietary/trustzone/microtrust/source/platform/microtrust.rc
        cp -R ../doujia_4m_tee_patch/* .
    fi
    #end
    ...
    YKQ_DOUJIA_TEE_SUPPORT_YK668=`grep "^YKQ_DOUJIA_TEE_SUPPORT_YK668" "$cust_file" |tail -1 |cut -d "=" -f 2 |tr -d " "`
    if [ "$YKQ_DOUJIA_TEE_SUPPORT_YK668" = "yes" ];then 
        cp -R ../yk668_cf9_ks810c_tee/* .
    fi

     ##add by zwl for 370 doujia tee 4m 
     YK_MICROTRUST_TEE_SUPPORT_37O_V4M=`grep "^YK_MICROTRUST_TEE_SUPPORT_37O_V4M" "$cust_file" |tail -1 |cut -d "=" -f 2 |tr -d " "`
     if [ "$YK_MICROTRUST_TEE_SUPPORT_37O_V4M" = "yes" ];then 
        cp -R ../doujia_37o_tee4m_common/* .
     fi
...
cfg_tee_str="CONFIG_TRUSTKERNEL_TEE_SUPPORT"
cfg_tee_rpmb_str="CONFIG_TRUSTKERNEL_TEE_RPMB_SUPPORT"
cfg_tee_on_support="CONFIG_TRUSTKERNEL_TEE_SUPPORT=y"
cfg_tee_off_support="# CONFIG_TRUSTKERNEL_TEE_SUPPORT is not set"
cfg_tee_rpmb_on_support="CONFIG_TRUSTKERNEL_TEE_RPMB_SUPPORT=y"
cfg_tee_rpmb_off_support="# CONFIG_TRUSTKERNEL_TEE_RPMB_SUPPORT is not set"
tee_debug=`grep -n "$cfg_tee_str" "$MY_DEBUG_DEFCONFIG" | head -1 | cut -d ":" -f 1`
tee_user=`grep -n "$cfg_tee_str" "$MY_DEFCONFIG" | head -1 | cut -d ":" -f 1`
tee_rpmb_debug=`grep -n "$cfg_tee_rpmb_str" "$MY_DEBUG_DEFCONFIG" | head -1 | cut -d ":" -f 1`
tee_rpmb_user=`grep -n "$cfg_tee_rpmb_str" "$MY_DEFCONFIG" | head -1 | cut -d ":" -f 1`
echo "tee_debug=$tee_debug"
echo "tee_user=$tee_user"
if [[ $cust_pingbo_sw_support == "yes" ]];then
echo "PINGBO TEE SW is YES"
if [[ $tee_user == "" ]];then
echo "$cfg_tee_on_support" >> $MY_DEFCONFIG
echo "$cfg_tee_rpmb_on_support" >> $MY_DEFCONFIG
else
##rpmb config last
sed -i "$tee_rpmb_user d" $MY_DEFCONFIG
sed -i "$tee_user d" $MY_DEFCONFIG
echo "$cfg_tee_on_support" >> $MY_DEFCONFIG
echo "$cfg_tee_rpmb_on_support" >> $MY_DEFCONFIG
fi
if [[ $tee_debug == "" ]];then
echo "$cfg_tee_on_support" >> $MY_DEBUG_DEFCONFIG
echo "$cfg_tee_rpmb_on_support" >> $MY_DEBUG_DEFCONFIG
else
sed -i "$tee_rpmb_debug d" $MY_DEBUG_DEFCONFIG
sed -i "$tee_debug d" $MY_DEBUG_DEFCONFIG
echo "$cfg_tee_on_support" >> $MY_DEBUG_DEFCONFIG
echo "$cfg_tee_rpmb_on_support" >> $MY_DEBUG_DEFCONFIG
fi
else
echo "PINGBO TEE SW is NO"
if [[ $tee_user == "" ]];then
echo "$cfg_tee_off_support" >> $MY_DEFCONFIG
echo "$cfg_tee_rpmb_off_support" >> $MY_DEFCONFIG
else
sed -i "$tee_rpmb_user d" $MY_DEFCONFIG
sed -i "$tee_user d" $MY_DEFCONFIG
echo "$cfg_tee_off_support" >> $MY_DEFCONFIG
...
if [[ $tee_debug == "" ]];then
echo "$cfg_tee_off_support" >> $MY_DEBUG_DEFCONFIG
echo "$cfg_tee_rpmb_off_support" >> $MY_DEBUG_DEFCONFIG
else
sed -i "$tee_rpmb_debug d" $MY_DEBUG_DEFCONFIG
sed -i "$tee_debug d" $MY_DEBUG_DEFCONFIG
echo "$cfg_tee_off_support" >> $MY_DEBUG_DEFCONFIG
...
PackSoftware()
{
   ...
   mkdir $DIR_FOLDER
#################################################below is MT6735 package ######################################################
MTK_PLATFORM_SUPPORT=`grep "^MTK_PLATFORM" "$cust_file" |tail -1|cut -d "=" -f 2 |tr -d " "`
if [ "$MTK_PLATFORM_SUPPORT" == "MT6735" ];then
   ...
   cp $BASE_NAME/out/target/product/$PROJ_NAME/obj/CGEN/APDB_MT6735_S01_alps-trunk-o1.bsp_W??.?? $BASE_NAME/$DIR_FOLDER
                                                       
if [ -f $BASE_NAME/out/target/product/$PROJ_NAME/system/etc/mddb/BPLGUInfoCustomAppSrcP_*_1_ltg_n ];then
   cp $BASE_NAME/out/target/product/$PROJ_NAME/system/etc/mddb/BPLGUInfoCustomAppSrcP_*_1_ltg_n $BASE_NAME/$DIR_FOLDER
fi
if [ -f $BASE_NAME/out/target/product/$PROJ_NAME/system/etc/mddb/BPLGUInfoCustomAppSrcP_*_1_lwg_n ];then
   cp $BASE_NAME/out/target/product/$PROJ_NAME/system/etc/mddb/BPLGUInfoCustomAppSrcP_*_1_lwg_n $BASE_NAME/$DIR_FOLDER
fi
if [ -f $BASE_NAME/out/target/product/$PROJ_NAME/vendor/etc/mddb/BPLGUInfoCustomAppSrcP_*_1_ltg_n ];then
   cp $BASE_NAME/out/target/product/$PROJ_NAME/vendor/etc/mddb/BPLGUInfoCustomAppSrcP_*_1_ltg_n $BASE_NAME/$DIR_FOLDER
fi                                                            
if [ -f $BASE_NAME/out/target/product/$PROJ_NAME/vendor/etc/mddb/BPLGUInfoCustomAppSrcP_*_1_lwg_n ];then
   cp $BASE_NAME/out/target/product/$PROJ_NAME/vendor/etc/mddb/BPLGUInfoCustomAppSrcP_*_1_lwg_n $BASE_NAME/$DIR_FOLDER
fi

#################################################below is MT6739 package ######################################################
else
    cp $BASE_NAME/out/target/product/$PROJ_NAME/preloader_$PROJ_NAME.bin $BASE_NAME/$DIR_FOLDER
if [ "$MTK_SECURITY_BOOT_SUPPORT" == "yes" ];then
