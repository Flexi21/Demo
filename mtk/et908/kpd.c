...
#if defined (VIBRATOR_GSENDOR)  // yx 20161014 for  close gsensor
void kpd_vibrator_gsensor_handler(int key_code)
{
	//for vibrate soon
	//upmu_set_rg_vibr_en(1); //xjl 20140526
	//mdelay(50);
...
#if defined(_MAIN2_CAM_ONLY_DETECT_LIGHT_)||defined(_MAIN2_CAM_SHELTER_DESIGN2_) //xen 20170211
...
/*********************************************************************/
#if defined(ET908_KEY_SUPPORT)
#define ET908_KEY_A    1
#define ET908_KEY_B    2
#define ET908_KEY_C    4
static unsigned char et_keys_status = 0; //for eint keys status

static void et_keys_bits_set(bool pressed, u16 keycode)
{
   static bool curr_queue_flg = 0,old_queue_flg = 0;

   if(pressed){
           if(keycode == ET908_KEY_A)//
           {
               et_keys_status |= ET908_KEY_A;
           }
           else if(keycode == ET908_KEY_A)
           {
               et_keys_status |= ET908_KEY_B;
           }
           else if(keycode == ET908_KEY_C)
           {
               et_keys_status |= ET908_KEY_C;
           }
   }
   else{
           if(keycode == ET908_KEY_A)//
           {
               et_keys_status = ~(et_keys_status & ET908_KEY_A);
           }
           else if(keycode == ET908_KEY_A)
           {
               et_keys_status = ~(et_keys_status & ET908_KEY_B);
           }
           else if(keycode == ET908_KEY_C)
           {
               et_keys_status = ~(et_keys_status & ET908_KEY_C);
           }
   }
   
   if(et_keys_status){
           curr_queue_flg = 1;     
           if(old_queue_flg ^ curr_queue_flg)
                      //wake up queue
   }
   else{    
           curr_queue_flg = 0;
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16; j++) {
			...
			pressed = !(new_state[i] & mask);
                        et_keys_bits_set(pressed, hw_keycode);
			if (kpd_show_hw_keycode)
...
#if defined(YK606_CUSTOMER_JIEKEXIN_V12_QHD_45)
#define EINTKEY_FUNCTION_SUPPORT
#endif

#ifdef EINTKEY_FUNCTION_SUPPORT
u32 keyeint1_irq_number = 0;
struct delayed_work keyeint1_eint_work;

static u8 kpd_keydet_state = 1; //HALL_FAR_AWAY;  //!KPD_PWRKEY_POLARITY;

static irqreturn_t kpd_keyeint_eint_handler(int irq, void *desc)
{
	disable_irq_nosync(keyeint1_irq_number);
	schedule_delayed_work(&keyeint1_eint_work, 0);
	return IRQ_HANDLED;
}
static void kpd_keyeint_handler(struct work_struct *work)
{
	bool pressed;
	kpd_keydet_state = !kpd_keydet_state;
	pressed = (kpd_keydet_state == 0);
	...
	if (pressed)  
	{
	   ...
	   input_report_key(kpd_input_dev, KEY_VOLUMEUP, 0);
	   input_sync(kpd_input_dev);
	   irq_set_irq_type(keyeint1_irq_number, IRQ_TYPE_LEVEL_HIGH);
	}
	else
	{
		irq_set_irq_type(keyeint1_irq_number, IRQ_TYPE_LEVEL_LOW);
		mdelay(100);
	}
	/* for detecting the return to old_state */
	enable_irq(keyeint1_irq_number);
}
u32 keyeint2_irq_number = 0;
struct delayed_work keyeint2_eint_work;
static u8 kpd_keydet_state2 = 1; //HALL_FAR_AWAY;  //!KPD_PWRKEY_POLARITY;
static irqreturn_t kpd_keyeint2_eint_handler(int irq, void *desc)
{
	disable_irq_nosync(keyeint2_irq_number);
	schedule_delayed_work(&keyeint2_eint_work, 0);
	return IRQ_HANDLED;
}
static void kpd_keyeint2_handler(struct work_struct *work)
{
	bool pressed;
	kpd_keydet_state2 = !kpd_keydet_state2;
	pressed = (kpd_keydet_state2 == 0);
	...
	if (pressed)  
	{
	   ...
	   input_report_key(kpd_input_dev, KEY_VOLUMEDOWN, 0);
	   input_sync(kpd_input_dev);
	   irq_set_irq_type(keyeint2_irq_number, IRQ_TYPE_LEVEL_HIGH);
	}
	else
	{
		irq_set_irq_type(keyeint2_irq_number, IRQ_TYPE_LEVEL_LOW);
		mdelay(100);
	}


	/* for detecting the return to old_state */
	enable_irq(keyeint2_irq_number);
}
#endif
#if defined(ET908_KEY_SUPPORT)
#define ET_KEYS_HWCODE_A        0x0002
#define ET_KEYS_HWCODE_B        0x0012
#define ET_KEYS_HWCODE_C        0x0112

#define MAIN2_DETECT_TIMEOUT   (HZ/10) //(HZ/2)	/*0.1 seconds*/
struct timer_list et_keys_poll_timer; 
struct work_struct et_keys_poll_work; 
struct workqueue_struct *et_keys_poll_queue;
//    del_timer_sync(&main2_detect_timer);
void et_keys_poll_timer_func(unsigned long data)
{
	queue_work(et_keys_poll_queue, &et_keys_poll_work);
}
static void et_key_queue_check_func(struct work_struct *work)
{
      static int curr_status[5];
      kpd_get_keymap_state(curr_status);

      
      mod_timer(&main2_detect_timer, jiffies + MAIN2_DETECT_TIMEOUT);
}

static void et_key_queue_init(void)
{
	init_timer(&et_keys_poll_timer);
	et_keys_poll_timer.expires = jiffies + MAIN2_DETECT_TIMEOUT;
	et_keys_poll_timer.function = &et_keys_poll_timer_func;
	et_keys_poll_timer.data = ((unsigned long)0);

	et_keys_poll_queue = create_singlethread_workqueue("et_keys_poll"); //xen 20170308
	INIT_WORK(&et_keys_poll_work, et_key_queue_check_func);
}
    del_timer_sync(&main2_detect_timer);
#endif
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	u32 ints[2] = { 0, 0 };
#endif
#if defined(EINTKEY_FUNCTION_SUPPORT) 
	struct device_node *key1_node = NULL;
	struct device_node *key2_node = NULL;
	u32 key1ints[2] = { 0, 0 };
	u32 key2ints[2] = { 0, 0 };
#endif
...
#if defined(HALL_FUNCTION_SUPPORT)|| defined(VIBRATOR_GSENDOR)        //added by xen for test HALL function 20130819  //VIBRATOR_GSENDOR yx for close gsensor 20161014
...
#endif
#if defined(EINTKEY_FUNCTION_SUPPORT)     //added by cjc for test eintkey function 20151217
	INIT_DELAYED_WORK(&keyeint1_eint_work, kpd_keyeint_handler);

        key1_node = of_find_compatible_node(NULL, NULL, "mediatek, key_1-eint");
	if (key1_node) {
		of_property_read_u32_array(key1_node, "debounce", key1ints,
					   ARRAY_SIZE(key1ints));
		gpio_set_debounce(key1ints[0], key1ints[1]);
		//kpd_print("ints[0] = %d, ints[1] = %d!!\n", ints[0], ints[1]);
		keyeint1_irq_number = irq_of_parse_and_map(key1_node, 0);
		//kpd_print("mhall_irq_number = %d\n", mhall_irq_number);
		if (!keyeint1_irq_number) {
			kpd_print("mhall irq_of_parse_and_map fail!!\n");
			return -EINVAL;
		}
		if (request_irq(keyeint1_irq_number, kpd_keyeint_eint_handler, IRQF_TRIGGER_LOW, "key_1-eint", NULL)) {
			kpd_print("IRQ LINE NOT AVAILABLE!!\n");
			return -EINVAL;
		}
		//enable_irq(keyeint1_irq_number);
	...
	else
	{
		...
		INIT_DELAYED_WORK(&keyeint2_eint_work, kpd_keyeint2_handler);

        key2_node = of_find_compatible_node(NULL, NULL, "mediatek, key_2-eint");
	if (key2_node) {
		of_property_read_u32_array(key2_node, "debounce", key2ints,
					   ARRAY_SIZE(key2ints));
		gpio_set_debounce(key2ints[0], key2ints[1]);
		//kpd_print("ints[0] = %d, ints[1] = %d!!\n", ints[0], ints[1]);
		keyeint2_irq_number = irq_of_parse_and_map(key2_node, 0); 
		//kpd_print("mhall_irq_number = %d\n", mhall_irq_number);
		if (!keyeint2_irq_number) {
			//kpd_print("mhall irq_of_parse_and_map fail!!\n");
			return -EINVAL;
		}
		if (request_irq(keyeint2_irq_number, kpd_keyeint2_eint_handler, IRQF_TRIGGER_LOW, "key_2-eint", NULL)) {
			//kpd_print("IRQ LINE NOT AVAILABLE!!\n");
			return -EINVAL;
		}
		//enable_irq(keyeint2_irq_number);
...
static int kpd_pdrv_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (call_status == 2) {
		...
		//kpd_wakeup_src_setting(0);  // modified by xen 20150430
