...
#ifndef VIRTUAL_GYROSCOPE_ALGORITHM
...
int GyroscopeSensor::batch(int handle, int flags,
...
#define MAX_REPORT_LETANCY_NS 0x7FFFFFFFFFFFFFFFULL
...
int GyroscopeSensor::reportFlush() {
...
int GyroscopeSensor::reportData(struct sensorData *data) {
    struct sensor_event event;
...
int GyroscopeSensor::getVirtualGyroAndReport() {
    struct sensorData outputData;
...
int GyroscopeSensor::enable(int32_t handle, int en) {
    ...
    if (!!en) {
        ...
        mSensorManager->batch(mSensorConnection, ID_ACCELEROMETER, samplingPeriodNs, mBatchReportLatencyNs);
        if (!accEnabled) {
            accEnabled = true;
        ...
        if (mSamplingPeriodNs < MIN_MAG_SAMPLE_DELAY_NS)
            ...
            samplingPeriodNs = mSamplingPeriodNs;
        mSensorManager->batch(mSensorConnection, ID_MAGNETIC, samplingPeriodNs, mBatchReportLatencyNs);
        if (!magEnabled) {
            magEnabled = true;
...
int GyroscopeSensor::setEvent(sensors_event_t *data) {
    struct sensorData inputData;
