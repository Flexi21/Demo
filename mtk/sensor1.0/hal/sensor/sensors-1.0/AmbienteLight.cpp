...
AmbiLightSensor::AmbiLightSensor()
    ...
    if (!mSensorSaved.getCalibrationInt32(ALS_CALI_SAVED_DIR, ALS_TAG_CALI, alsCali, 1))
        memset(alsCali, 0, sizeof(alsCali));
    ALOGI("read alsCali: [%d]\n", alsCali[0]);
    strlcpy(&input_sysfs_path[input_sysfs_path_len], "alscali", sizeof(input_sysfs_path) - input_sysfs_path_len);
    fd = TEMP_FAILURE_RETRY(open(input_sysfs_path, O_RDWR));
    if (fd >= 0) {
        TEMP_FAILURE_RETRY(write(fd, alsCali, sizeof(alsCali)));
        close(fd);
    } else
        ALOGE("no cali attr\n");
...
int AmbiLightSensor::setDelay(int32_t handle, int64_t ns)
{
    ...
    strlcpy(&input_sysfs_path[input_sysfs_path_len], "alsdelay", sizeof(input_sysfs_path) - input_sysfs_path_len);
    fd = TEMP_FAILURE_RETRY(open(input_sysfs_path, O_RDWR));
    if (fd < 0) {
...
void AmbiLightSensor::processEvent(struct sensor_event const *event)
{
    ...
    if (event->flush_action == FLUSH_ACTION) {
        ...
        alsCali[0] = event->word[0];
        ALOGI("write alsCali: [%d]\n", alsCali[0]);
        mSensorSaved.saveCalibrationInt32(ALS_CALI_SAVED_DIR, ALS_TAG_CALI, alsCali, 1);
    } else
        ALOGE("unknow action\n");
}
