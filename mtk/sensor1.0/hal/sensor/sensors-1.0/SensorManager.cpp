...
int SensorManager::Info::setBatchParamsForIdent(SensorConnection *connection, int flags,
    ...
    if (index < 0) {
        ALOGE("setBatchParamsForIdent(connection=%p, period_ns=%lld timeout=%lld)",
    ...
    if (enabled) {
        ...
        if (info.removeBatchParamsForIdent(connection) >= 0) {
            if (info.numActiveClients() == 0) {
                ...
                ALOGI_IF(DEBUG_CONNECTIONS, "\tctivate>>> actuating h/w batch %d %d %lld %lld",
...
int SensorManager::batch(SensorConnection *connection, int32_t sensor_handle,
    ...
    ALOGI_IF(DEBUG_CONNECTIONS, "batch handle(%d) sampling_period_ns(%lld) max_report_latency_ns(%lld).",
    ...
    ALOGI_IF(DEBUG_CONNECTIONS,
            "\tbatch>>> curr_period=%lld min_period=%lld curr_timeout=%lld min_timeout=%lld",
