...
#define SENSOR_SAVED_NVCFG_ROOT_DIR       "/vendor/nvcfg"
#define SENSOR_SAVED_NVCFG_SENSOR_DIR     "/vendor/nvcfg/sensor"
#define SENSOR_SAVED_DATA_MISC_SENSOR_DIR "/data/misc/sensor"
...
static bool vendorNvcfgPathValided() {
    ...
    if (access(SENSOR_SAVED_NVCFG_ROOT_DIR, F_OK)) {
        ...
        mkdir(SENSOR_SAVED_DATA_MISC_SENSOR_DIR, 0774);
    } else {
        ALOGE("(%s) exist\n", SENSOR_SAVED_NVCFG_ROOT_DIR);
...
SensorSaved::SensorSaved() {
    if (vendorNvcfgPathValided())
        ...
        strlcpy(sensorSavedPath, SENSOR_SAVED_DATA_MISC_SENSOR_DIR, sizeof(sensorSavedPath));
