...
/* MTK sensor type */
...
#ifdef CUSTOM_KERNEL_ACCELEROMETER_UNCALIBRATED
    memset(&sensor, 0, sizeof(struct sensor_t));
    sensor.name = ACCELEROMETER_UNCALIBRATED;
    sensor.vendor = ACCELEROMETER_UNCALIBRATED_VENDER;
    sensor.version = ACCELEROMETER_UNCALIBRATED_VERSION;
    sensor.handle = ID_ACCELEROMETER_UNCALIBRATED + ID_OFFSET;
    sensor.type = SENSOR_TYPE_ACCELEROMETER_UNCALIBRATED;
    sensor.maxRange = ACCELEROMETER_UNCALIBRATED_RANGE;
    sensor.resolution = ACCELEROMETER_UNCALIBRATED_RESOLUTION;
    sensor.power = ACCELEROMETER_UNCALIBRATED_POWER;
    sensor.minDelay = ACCELEROMETER_UNCALIBRATED_MINDELAY;
    sensor.fifoReservedEventCount = ACCELEROMETER_UNCALIBRATED_FIFO_RESERVE_COUNT;
    sensor.fifoMaxEventCount = ACCELEROMETER_UNCALIBRATED_FIFO_MAX_COUNT;
    sensor.stringType = SENSOR_STRING_TYPE_ACCELEROMETER_UNCALIBRATED;
    sensor.maxDelay = ACCELEROMETER_UNCALIBRATED_MAXDELAY;
    sensor.flags = ACCELEROMETER_UNCALIBRATED_FLAGS;
    mSensorList.push_back(sensor);
#endif
    ...
    mSensorList.push_back(sensor);
#endif
    ALOGI("defaultSensorListCount=%d\n", mSensorList.size());
}
...
SensorList::SensorList() {
    ...
    if (lib_handle)
        dlclose(lib_handle);
    ALOGI("newSensorListCount=%d\n", mSensorList.size()); //zwl
}
