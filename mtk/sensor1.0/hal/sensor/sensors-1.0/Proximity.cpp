...
ProximitySensor::ProximitySensor()
    ...
    if (!mSensorSaved.getCalibrationInt32(PS_CALI_SAVED_DIR, PS_TAG_CALI, psCali, 2))
        memset(psCali, 0, sizeof(psCali));
    ALOGI("read cali [high, low]: [%d, %d]\n", psCali[0], psCali[1]);
    strlcpy(&input_sysfs_path[input_sysfs_path_len], "pscali", sizeof(input_sysfs_path) - input_sysfs_path_len);
    fd = TEMP_FAILURE_RETRY(open(input_sysfs_path, O_RDWR));
    if (fd >= 0) {
        TEMP_FAILURE_RETRY(write(fd, psCali, sizeof(psCali)));
        close(fd);
    } else
        ALOGE("no cali attr\n");
}
...
int ProximitySensor::readEvents(sensors_event_t* data, int count)
{
    ...
    while (count && mSensorReader.readEvent(&event)) {
        ...
        if (/*mEnabled &&*/ (mPendingEvent.distance >= 0) && (event->flush_action <= FLUSH_ACTION)) {
            *data++ = mPendingEvent;
            numEventReceived++;
            count--;
...
void ProximitySensor::processEvent(struct sensor_event const *event)
{
    ...
    if (event->flush_action == FLUSH_ACTION) {
        ...
        psCali[0] = event->word[0];
        psCali[1] = event->word[1];
        ALOGI("write [high, low]: [%d, %d]\n", psCali[0], psCali[1]);
        mSensorSaved.saveCalibrationInt32(PS_CALI_SAVED_DIR, PS_TAG_CALI, psCali, 2);
    } else
        ALOGE("unknow action\n");
}
