...
int custom_function_red(unsigned int level)
{
	pr_debug("\nxxxx--custom_function_red\n");
        #if 0//defined(ET918_CONFIG)  
        et_gpio_set_value(ET_GPIO_RLED, level);
        #endif
        
        #if 0//defined(ET_BUILD_ENG)
        et_gpio_get_value(ET_GPIO_RLED);
        #endif
	return 0;
}
int custom_function_green(unsigned int level)
{
	pr_debug("\nxxxx--custom_function_green\n");
        #if defined(ET918_CONFIG)
        et_gpio_set_value(ET_GPIO_GLED, level);
        #endif
        
        #if defined(ET_BUILD_ENG)
        et_gpio_get_value(ET_GPIO_GLED);
        #endif
	return 0;
}

int custom_function_blue(unsigned int level)
{
        pr_debug("\nxxxx--custom_function_blue\n");
        #if defined(ET918_CONFIG)
        et_gpio_set_value(ET_GPIO_BLED, level);
        #endif

        #if defined(ET_BUILD_ENG)
        et_gpio_get_value(ET_GPIO_BLED);
        #endif
	return 0;
}
#endif
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_CUST_LCM:
		...
		if ((strcmp(cust->name, "red")) == 0)
                {
			return custom_function_red(level);
		}
		else if ((strcmp(cust->name, "green") == 0)) 
                {
			return custom_function_green(level);
	 	}
		else if((strcmp(cust->name, "blue") == 0)) 
		{
			return custom_function_blue(level);
		}
                #else
		if (strcmp(cust->name, "lcd-backlight") == 0)
			bl_brightness_hal = level;
