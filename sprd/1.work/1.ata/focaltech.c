...
#if defined(JM_TOUCHSCREEN_UPDATE)//Leo 20200107
#define SPRD_AUTO_UPGRADE
#endif
 ...
 struct Upgrade_Info fts_updateinfo[] =
{
	...
	{0x64,"FT6336GU",TPD_MAX_POINTS_2,AUTO_CLB_NONEED,10, 10, 0x79, 0x1c, 10, 2000}, //,"FT6336GU"
};
...
// Leo add for tp ata test 20200218
static ssize_t input_name_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", FOCALTECH_TS_NAME);
}

static struct kobj_attribute input_name_attr = {
	.attr = {
		.name = "input_name",
		.mode = S_IRUGO,
	},
	.show = &input_name_show,
};

static struct attribute *input_name_attrs[] = {
	&input_name_attr.attr,
	NULL
};

static struct attribute_group input_name_attr_group = {
	.attrs = input_name_attrs,
};

static void sprd_ata_info_init(void)
{
	int ret = 0;
	struct kobject *input_name_kobj;

	pr_info("%s Entry!\n",__func__);

	input_name_kobj = kobject_create_and_add("touchscreen", NULL);
	if (input_name_kobj)
		ret = sysfs_create_group(input_name_kobj, &input_name_attr_group);

	if (!input_name_kobj || ret)
		pr_err("failed to create input_name_kobj\n");
}
// Leo add end 202002198
...
static int fts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
  ...
  /*set correct i2c addr*/
	...
	#if defined(FT_HAVE_KEYS) //Leo 20200116
...
if (ft_rw_iic_drv_init(client) < 0)	
{
	...
	sprd_ata_info_init(); //Leo 20200218
...
#ifndef CONFIG_HAS_EARLYSUSPEND //Leo 20200104
static int fts_suspend(struct device *dev)
{
	int ret;
	pr_info("leo fts_suspend\n");
	ret = fts_write_reg(this_client, FTS_REG_PMODE, PMODE_HIBERNATE);
	if(ret){
		PRINT_ERR("==fts_suspend==  fts_write_reg fail\n");
	}
